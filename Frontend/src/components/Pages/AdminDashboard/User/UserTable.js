...
export default function UserTable() {
    ...
    const columns = React.useMemo(
        () => [
            ...
            },
            {
                ...
                Cell: ({ cell: { value } }) => (
                    <div className="">
                        <Link to={"/admin/dashboard/user/details/" + value}> <button className="btn view_btn">View Details</button></Link>
                    </div >
                )
            },
        ], []);
    ...
    useEffect(() => {
        async function getData() {
            await axios
                .get(`${process.env.REACT_APP_BACKEND_URL}/admin/user/details`)
                .then((response) => {
    ...
    // const userData = React.useMemo(() => data, []);
